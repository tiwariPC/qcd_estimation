...
file_in = ROOT.TFile('rootFiles/step1/step1_qcdDphi_'+args.year+'.root', 'READ')
file_out = ROOT.TFile('rootFiles/step2/step2_qcdDphi_'+args.year+'.root', 'RECREATE')
...
for cr in crs:
    ...
    for key in range(1,total_bins+1):
        qcdDphiCTS[key] = ROOT.TH1F('qcdDphiCTS_'+cr+'_bin'+str(key),'qcdDphiCTS_'+cr+'_bin'+str(key),len(binx_)-1,array('d', binx_))
    qcdDphiCTS_tot = ROOT.TH1F('qcdDphiCTS_'+cr+'_tot','qcdDphiCTS_'+cr+'_tot',len(binx_)-1,array('d', binx_))
    qcdDphiCTS_tot2 = ROOT.TH1F('qcdDphiCTS_'+cr+'_tot2','qcdDphiCTS_'+cr+'_tot2',len(binx_)-1,array('d', binx_))
    DYJets_Hits = []
    ZJets_Hits = []
    WJets_Hists = []
    GJets_Hists = []
    ...
    SMH_Hists = []
    MET_Hists = []
    for histname_ in hist_list:
        histname = histname_+'_'+cr
        histo_ = file_in.Get(histname)
        histo_total_weight = file_in.Get('h_total_mcweight_'+histname_)
        ...
        elif 'MET-Run' in histname and ('ZmumuQCDCR_2j' in cr or 'ZmumuQCDCR_3j' in cr or 'WmunuQCDCR_1b' in cr or 'TopmunuQCDCR_2b' in cr or 'QCDbCR_1b' in cr or 'QCDbCR_2b' in cr ):
            MET_Hists.append(histo_)
        elif ('SingleElectron-Run' in histname or 'EGamma-Run' in histname) and ('ZeeQCDCR_2j' in cr or 'ZeeQCDCR_3j' in cr or 'WenuQCDCR_1b' in cr or 'TopenuQCDCR_2b' in cr):
                print(histname, cr)
                MET_Hists.append(histo_)
    ...
    temp_totalBkg = Tot_Bkg.Clone()
    temp_totalBkg.Add(QCD)

    qcd_contribution = (QCD.Integral()/temp_totalBkg.Integral())

    if abs(qcd_contribution) <= 0.01:
    ...
    for i in range(1, QCD.GetXaxis().GetNbins() + 1):
        for j in range(1, 5):
            original_content = QCD.GetBinContent(i, j)
            original_error = QCD.GetBinError(i, j)
            if original_content <= 0.0:
                QCD.SetBinContent(i, j, 0.0)
                QCD.SetBinError(i, j, 0.0)
    print("Y-Axis Bins",QCD.GetYaxis().GetNbins())

    qcd_contribution = (QCD.Integral()/temp_totalBkg.Integral())
    if qcd_contribution < 0.005:
        print('qcd_contribution: ', qcd_contribution)
